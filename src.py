...
class SimplexPredictor(BaseEstimator, RegressorMixin):
    ...
    def fit(self, X):
        self.X_ = pd.DataFrame(
            {"Time": list(range(len(X.values))), "x": X.values.flatten()}
        )
        self.lib_start = 1
        self.lib_end = int(len(X.values) * 0.8)
        self.embed_start = self.lib_end + 1
        self.embed_end = len(X.values)
        self.pred_start = len(X.values) + 1
        return self
    ...
    def predict(self, fh):
        ...
        embeddimtable = EmbedDimension(
            dataFrame=self.X_,
            lib=f"{self.lib_start} {self.lib_end}",
            pred=f"{self.embed_start} {self.embed_end}",
            columns="x",
            showPlot=False,
        )
        self.embed_dim = embeddimtable[
            embeddimtable["rho"] == embeddimtable["rho"].max()
        ]["E"]
        self.res = Simplex(
            dataFrame=self.X_,
            lib=f"{self.lib_start} {self.embed_end}",
